...
public class TestProductControllerCorrect {
    @Autowired
    ...
    void testCorrect() throws Exception {
        ...
        when(productCorrect.execute(any(), any())).thenReturn(productResponse);
        ...
        var productEntityModel = EntityModel.of(productResponseDTO, selfLink, allLink, producerLink, productTypeLink);
        when(productAssembler.toModel(any())).thenReturn(productEntityModel);

        var template = "{\"name\": \"%s\", \"weight\": \"%s\", \"producer_id\": \"%s\", \"producer_type_id\": \"%s\" }";
        var requestBody = String.format(template, name, weight, producerId, productTypeId);
        var request = String.format("/products/%s", id);
        mockMvc.perform(put(request).contentType(MediaType.APPLICATION_JSON).content(requestBody))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.id").value(id.toString()))
                .andExpect(jsonPath("$.name").value(name))
                .andExpect(jsonPath("$.weight").value(weight.toString()))
